...
_DEFAULT_WIND_DF = os.path.join(_DATA_DIR, 'wind_historical_synthetic.csv')
...
class AORVAEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.wind_field = WindField3D(self.voxel_grid, reference_height_m=10.0, alpha=0.25)
    ...
    def _step_physics(self, desired_vel: np.ndarray) -> None:
        ...
        u, v, w = self.wind_field.sample(ix, iy, iz)
        ...
        thrust_power = float(np.sum(effective_vel ** 2)) * 0.5 + 50.0
    ...
    def _compute_step_reward(self, desired_vel: np.ndarray) -> float:
        ...
        """
        # --- w2 * R_k : ground risk ---
        # Placeholder until ABS population density is rasterised into
        # voxel_grid.metadata. For now: altitude-based proxy (risk high
        # near ground, lower at cruise altitude).
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        ua, va, wa = self.wind_field.sample(lx, ly, lz)

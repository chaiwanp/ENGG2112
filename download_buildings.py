...
# Try GPKG first
try:
    buildings_clean.to_file(os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.gpkg'), driver='GPKG')
    ...
    # Fallback to GeoJSON
    try:
        buildings_clean.to_file(os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.geojson'), driver='GeoJSON')
        ...
        with open(os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.pkl'), 'wb') as f:
...
plt.tight_layout()
plt.savefig(os.path.join(_OUTPUT_DIR, 'buildings_map.png'), dpi=300, bbox_inches='tight')
print(f"[OK] Saved visualization to {os.path.join(_OUTPUT_DIR, 'buildings_map.png')}")
plt.show()
...
_gpkg = os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.gpkg')
_geojson = os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.geojson')
_pkl = os.path.join(_DATA_DIR, 'buildings_westmead_liverpool.pkl')
if os.path.exists(_gpkg):
    print(f"  [OK] {_gpkg}")
elif os.path.exists(_geojson):
    print(f"  [OK] {_geojson}")
else:
    print(f"  [OK] {_pkl}")
print(f"  [OK] {os.path.join(_OUTPUT_DIR, 'buildings_map.png')}")
print("="*60)

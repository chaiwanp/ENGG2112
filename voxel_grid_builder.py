...
class VoxelGrid3D:
    def __init__(self, bounds, voxel_size_m=50, voxel_size_z_m=None, max_height_m=500):
        """
        Create 3D voxel grid

        Parameters:
        - bounds: (min_lon, min_lat, max_lon, max_lat)
        - voxel_size_m: horizontal (XY) voxel size in meters
        - voxel_size_z_m: vertical (Z) voxel size in meters; defaults to voxel_size_m
        - max_height_m: maximum altitude
        """
        self.voxel_size_m = voxel_size_m
        self.voxel_size_z_m = voxel_size_z_m if voxel_size_z_m is not None else voxel_size_m
        ...
        self.nz = int(np.ceil(max_height_m / self.voxel_size_z_m))
    ...
    def latlon_to_grid(self, lat, lon, alt_m=0):
        ...
        z = int(alt_m / self.voxel_size_z_m)
    ...
    def add_buildings(self, buildings_gdf):
        ...
        for idx, building in buildings_gdf.iterrows():
            ...
            z_max = int(height / self.voxel_size_z_m)
    ...
    def add_bankstown_nfz(self, radius_m=2000, max_alt_m=762):
        """Mark Bankstown Airport CTR as no-fly (CASA CASR Part 101).

        Bankstown ARP: -33.9244, 150.9883
        Radius reduced to 2 km (vs standard 3 NM) under CASA medical drone exemption.
        Altitude ceiling: 2500 ft = 762 m.
        """
        arp_lat, arp_lon = -33.9244, 150.9883
        print(f"Adding Bankstown Airport NFZ (r={radius_m}m, ceiling={max_alt_m}m)...")

        z_end = min(int(max_alt_m / self.voxel_size_z_m), self.nz)

        for xi in range(self.nx):
            for yi in range(self.ny):
                lat, lon, _ = self.grid_to_latlon(xi, yi, 0)
                dlat_m = (lat - arp_lat) * self.m_per_deg_lat
                dlon_m = (lon - arp_lon) * self.m_per_deg_lon
                if dlat_m ** 2 + dlon_m ** 2 <= radius_m ** 2:
                    for z in range(z_end):
                        self.grid[xi, yi, z] = 1
                        self.metadata[xi, yi, z] = 2  # No-fly zone

        marked = np.sum(self.metadata == 2)
        print(f"Bankstown NFZ: {marked:,} voxels marked as no-fly.")
    ...
    def add_no_fly_zones(self, zones_gdf, min_alt_m=0, max_alt_m=500):
        ...
        for idx, zone in zones_gdf.iterrows():
            ...
            x_start, y_start, _ = self.latlon_to_grid(miny, minx, 0)
            x_end, y_end, _ = self.latlon_to_grid(maxy, maxx, 0)
            z_start = int(min_alt_m / self.voxel_size_z_m)
            z_end = int(max_alt_m / self.voxel_size_z_m)
    ...
    def add_population_density(self, density_map):
        """Store a 2-D population density array (nx, ny) on the grid."""
        self.population_density = density_map.astype(np.float32)
    ...
    def save(self, filepath):
        ...
        data = {
            ...
            'voxel_size_z_m': self.voxel_size_z_m,
            ...
            'population_density': getattr(self, 'population_density', None),
        }
    ...
    def load(cls, filepath):
        ...
        grid_obj.voxel_size_z_m = data.get('voxel_size_z_m', data['voxel_size_m'])
        ...
        grid_obj.population_density = data.get('population_density', None)

...
# SA2 regions belonging to Liverpool City LGA (ABS 2021 ASGS).
# These are exempted from ground-risk penalisation because Liverpool Hospital
# is the authorised delivery destination — the drone must fly over/into this
# area to complete its mission, so penalising it contradicts the objective.
_LIVERPOOL_LGA_SA2 = {
    "Liverpool - East", "Liverpool - West",
    "Warwick Farm", "Casula", "Chipping Norton - Moorebank",
    "Hinchinbrook", "Green Valley", "Lurnea - Cartwright",
    "Ashcroft - Busby - Miller", "West Hoxton - Middleton Grange",
    "Cecil Hills", "Prestons", "Edmondson Park",
    "Hoxton Park - Carnes Hill - Horningsea Park",
    "Leppington - Catherine Field", "Holsworthy - Wattle Grove",
    "Austral - Greendale", "Denham Court - Bardia",
}
...
def download_population_density(
        ...
        exempt_liverpool_lga: bool = True):
    """
    ...
    ----------
    clip_bounds : (west, south, east, north)
    exempt_liverpool_lga : bool
        If True (default), zero out density and risk weight for all SA2
        regions in Liverpool City LGA.  Liverpool Hospital is the
        authorised delivery destination, so the corridor into it must not
        be penalised by the ground-risk term.
    ...
    if exempt_liverpool_lga:
        mask = result['SA2_NAME_2021'].isin(_LIVERPOOL_LGA_SA2)
        result.loc[mask, 'density_ppl_km2'] = 0.0
        result.loc[mask, 'risk_weight']     = 0.0
        n_exempted = int(mask.sum())
        print(f"  Liverpool LGA exempted: {n_exempted} SA2 region(s) set to zero density")
